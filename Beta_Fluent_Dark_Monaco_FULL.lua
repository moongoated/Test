...
--[[ =========================================================
     FLUENT UI – DARK MONACO BACKGROUND PATCH
     Synapse X / KRNL style
     Image ID: 104535323405239
     Non-destructive (keeps 100% of original file)
========================================================= ]]
...
task.spawn(function()
    ...
    -- Wait until Fluent ScreenGui exists
    local fluentGui
    for i = 1, 300 do
        for _, v in ipairs(gui:GetChildren()) do
            if v:IsA("ScreenGui") and v.Name:lower():find("fluent") then
                fluentGui = v
                break
            end
        end
    ...
    -- Wait until the main window frame exists
    local container
    for i = 1, 300 do
        container = fluentGui:FindFirstChildWhichIsA("Frame", true)
        if container then break end
        task.wait(0.1)
    end
    if not container then return end

    if container:FindFirstChild("DarkMonacoBackground") then return end

    -- Background image
    local bg = Instance.new("ImageLabel")
    bg.Name = "DarkMonacoBackground"
    bg.Parent = container
    bg.Image = "rbxassetid://104535323405239"
